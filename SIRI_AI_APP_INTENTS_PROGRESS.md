...
- Removed prefilled sentinel/default values. A generic hormone-level request leaves `hormone` unset so the intent uses the user's explicitly confirmed HRT Profile rather than silently selecting estradiol; an explicit estradiol/testosterone parameter overrides the profile, and no confirmed profile produces a setup prompt instead of a guess. The planned-dose shortcut likewise asks for an active dose instead of receiving a stale placeholder.
...
The Xcode 27-built app was installed and launched on iOS 18.5, iOS 26.1, and iOS 27.0 simulators. Release builds succeeded for both the iOS Simulator and arm64 `iphoneos`. The app artifact has a 16.0 minimum OS version, weak-links `FoundationModels.framework`, and contains no Foundation Models `Attachment` symbol, so loading the app on iOS 18 or iOS 26 does not require or instantiate that framework.

The Debug-only OCR fallback suite was enabled explicitly and run on iOS 18.5, iOS 26.1, and iOS 27.0 simulators. All 13 checks passed on each OS, covering Chinese upload/scan OCR, leading-flag artifacts, sparse and split tables, specimen-field grounding, English and patient-portal layouts, international date/number formats, generic non-hormone rows, column-order variants, administrative-field noise, and flexible OCR-damaged concentration units. The synthetic image pipeline also passed with seven analytes on all three OS versions. On iOS 27, a simulator model guardrail error cleanly fell back to verified OCR instead of crashing or retrying an equivalent image request.

The generated App Intents metadata keeps `hormone` optional so an explicit Siri/Shortcut value wins while a generic request uses the user's confirmed HRT Profile; if no profile has been confirmed, the intent refuses to guess and asks the user to choose one in the app. `doseOption` remains required with no stale placeholder. SSU/NLU assets are generated for English, Simplified Chinese, and Traditional Chinese.
...
9. Confirm an E2 HRT Profile, then run `Check my hormone level in HRT Recorder` and verify it uses estradiol. Switch the profile to T and verify the generic request uses testosterone. Then try an explicit `Check estradiol level in HRT Recorder` and verify the explicit hormone overrides the profile. With no confirmed profile, verify the generic request asks the user to choose an HRT type in the app and never defaults silently. The response is an estimate from local recorded data, not a clinical measurement.
